...
public class BoardDAO {
	...
	public int addBoard(BoardVO vo) {
		...
		try {
			...
			// 1. 쿼리문 준비
			String sql="insert into b_board(seq,title,nickname,content,userid) values("
	...
	public ArrayList<BoardVO> getPopBoardList() {
		...
		try {
			...
			conn=jdbcConnection.getConnection();
			// 1. 쿼리문 준비
			String sql = "select * from b_board where cnt=(SELECT max(cnt) FROM b_board) ORDER BY seq DESC";
			// 2. prepareStatement
			stmt = conn.prepareStatement(sql);
			// 3. ? 매핑
			// 4. 실행
	...
	// #2-2. 검색 메서드
	public ArrayList<BoardVO> searchBoardList(String condition, String search) {
		ArrayList<BoardVO> boardList=null;
		try {
			// 0. DB 연결
			conn=jdbcConnection.getConnection();
			// 1. 쿼리문 준비
			String sql="select * from b_board where "+condition+" like '%'||?||'%' order by seq desc";
			// 2. prepareStatement
			stmt=conn.prepareStatement(sql);
			// 3. ? 매핑
